...
export class ConversationLifecycleControl {
  ...
  async post(route: string, body: Record<string, unknown>): Promise<LifecycleResponse> {
  ...
  async releaseSettled(
    ...
    for (const conversation of targets) {
      try {
        const response = await this.request("/api/state/conversations/release-worktree", {
          method: "POST",
          headers: { "content-type": "application/json" },
          body: JSON.stringify({ threadId: conversation.id, confirm: true }),
        });
        const result = (await response.json()) as LifecycleResponse;
        if (!response.ok) throw new Error(result.error ?? "Managed worktree release failed.");
