  ...
  app
    ...
    .then(async () => {
      ...
      const browser = new SharedBrowserManager(
        join(applicationRoot, "dist-electron", "preload.cjs"),
      );
      ...
      remoteEnvironments = new RemoteEnvironmentManager(
        ...
        (id) => {
          if (activeRemoteEnvironmentId !== id || !window || !localApplicationUrl || shuttingDown)
            return;
          ...
          void window.loadURL(localApplicationUrl).catch((error: unknown) => {
            console.error(
              "The local workbench could not be restored after the SSH connection closed.",
              error,
            );
          });
        },
      ...
      ipcMain.handle(REGISTER_BROWSER_VIEW_CHANNEL, (event, value: unknown) => {
        if (
          !isLocalApplicationWindow(event) ||
          !window ||
          typeof value !== "object" ||
          value === null
        )
      ...
      ipcMain.handle(REMOTE_ENVIRONMENT_SAVE_CHANNEL, async (event, input: unknown) => {
        if (!isLocalApplicationWindow(event) || !remoteEnvironments)
          throw new Error("The desktop window is unavailable.");
        if (!input || typeof input !== "object" || Array.isArray(input))
          throw new Error("A remote environment is required.");
      ...
      ipcMain.handle(REMOTE_ENVIRONMENT_REMOVE_CHANNEL, async (event, id: unknown) => {
        if (!isLocalApplicationWindow(event) || !remoteEnvironments || typeof id !== "string")
          return;
      ...
      ipcMain.handle(REMOTE_ENVIRONMENT_CONNECT_CHANNEL, async (event, value: unknown) => {
        if (!isLocalApplicationWindow(event) || !remoteEnvironments || !window)
          throw new Error("The desktop window is unavailable.");
        if (!value || typeof value !== "object" || Array.isArray(value))
          throw new Error("A remote environment id is required.");
        ...
        try {
          ...
          if (
            targetOrigin !== previousRemoteOrigin &&
            targetOrigin !== new URL(localApplicationUrl).origin
          ) {
          ...
          throw error instanceof Error
            ? error
            : new Error("The remote workbench could not be loaded.");
        }
        if (previousEnvironmentId && previousEnvironmentId !== target.id) {
          await remoteEnvironments.disconnect(previousEnvironmentId);
          if (previousRemoteOrigin && previousRemoteOrigin !== targetOrigin)
            approvedOrigins?.delete(previousRemoteOrigin);
        }
        return target;
      });
      ...
      ipcMain.handle(REMOTE_ENVIRONMENT_CONFIRM_CHANNEL, async (event) => {
        if (
          !isActiveRemoteApplicationWindow(event) ||
          !remoteEnvironments ||
