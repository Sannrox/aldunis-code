...
several machines. The work lifecycle
(`docs/work-lifecycle.md`, "Parallel delivery lanes") owns the
...
1. If the user named Issues, those are the candidate lanes. Otherwise list
   open `status:ready` Issues and run `issue-lane.sh check` on each. There is
   no `advance-issue-frontier` skill in this repository; do not invent one.

   ```sh
   gh issue list --label status:ready --state open --json number,title,assignees
...
between selection and now: drop the candidate, do not rename the branch,
and refill the slot from the remaining ready Issues if capacity remains.
...
- procedure: run deliver-ready-issue from bound/implement onward inside the
  worktree; readiness, claim, and isolation are done.
...
3. After each landing, run `git fetch --prune origin` and re-list open
   `status:ready` Issues. Ask the remaining lanes to check `mergeable` and their
   tests against the new `main`. Refresh a lane onto `main` only for a conflict,
   a failing gate, an explicit request, or a sibling landing on a shared
   surface.
