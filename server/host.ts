...
async function handleApi(
  ...
  try {
    ...
    if (route === "/api/autonomy/load") {
      ...
      const runs = snapshot.runs.filter((run) => run.projectId === null || projectIds.has(run.projectId));
    ...
    if (route === "/api/autonomy/gardener/start") {
      ...
      if (body.goal !== undefined && typeof body.goal !== "string") throw new AutonomyError("The gardener goal is invalid.");
      const standingOrderIds = body.standingOrderIds === undefined
        ? undefined
        : Array.isArray(body.standingOrderIds) && body.standingOrderIds.every((item) => typeof item === "string")
    ...
    if (route === "/api/autonomy/runs/cancel") {
      if (remoteRequest || managedHost) throw new AutonomyError("Remote clients cannot cancel autonomy runs.", 403);
    ...
    if (route === "/api/autonomy/runs/resume") {
      if (remoteRequest || managedHost) throw new AutonomyError("Remote clients cannot resume autonomy runs.", 403);
      const body = await readJson(request) as { runId?: unknown };
      if (typeof body.runId !== "string") throw new AutonomyError("An autonomy run is required.");
    ...
    if (route === "/api/autonomy/heartbeats/create") {
      if (remoteRequest || managedHost) throw new AutonomyError("Remote clients cannot create heartbeats.", 403);
      const body = await readJson(request) as {
        name?: unknown;
        projectId?: unknown;
        worktree?: unknown;
      ...
      if (typeof body.name !== "string" || typeof body.goal !== "string" || typeof body.everySeconds !== "number") {
        throw new AutonomyError("Heartbeat name, goal, and interval are required.");
      }
      if (body.projectId !== undefined && body.projectId !== null && typeof body.projectId !== "string") throw new AutonomyError("Heartbeat project is invalid.");
      if (body.worktree !== undefined && body.worktree !== null && typeof body.worktree !== "string") throw new AutonomyError("Heartbeat worktree is invalid.");
      sendJson(response, 200, await autonomy.addHeartbeat({
        name: body.name,
    ...
    if (route === "/api/autonomy/heartbeats/update") {
      if (remoteRequest || managedHost) throw new AutonomyError("Remote clients cannot update heartbeats.", 403);
      const body = await readJson(request) as {
        id?: unknown;
        name?: unknown;
        projectId?: unknown;
      ...
      const current = (await state.load()).heartbeatMonitors.find((monitor) => monitor.id === body.id);
      ...
      const updated = parseHeartbeatMonitor({
        ...
        ...(typeof body.worktree === "string" || body.worktree === null ? { worktree: body.worktree } : {}),
    ...
    if (route === "/api/autonomy/heartbeats/delete") {
      if (remoteRequest || managedHost) throw new AutonomyError("Remote clients cannot delete heartbeats.", 403);
      const body = await readJson(request) as { id?: unknown };
      if (typeof body.id !== "string") throw new AutonomyError("A heartbeat is required.");
    ...
    if (route === "/api/autonomy/heartbeats/run-now") {
      if (remoteRequest || managedHost) throw new AutonomyError("Remote clients cannot run heartbeats.", 403);
      const body = await readJson(request) as { id?: unknown };
      if (typeof body.id !== "string") throw new AutonomyError("A heartbeat is required.");
      const monitor = (await state.load()).heartbeatMonitors.find((candidate) => candidate.id === body.id);
      if (!monitor) throw new AutonomyError("Heartbeat not found.", 404);
      const run = await autonomy.startHeartbeat(monitor, "manual");
      await state.saveAutonomyRecords({ heartbeatMonitors: [{ ...monitor, lastRunAt: new Date().toISOString(), lastRunId: run.id, lastStatus: "queued", updatedAt: new Date().toISOString() }] });
      sendJson(response, 202, run);
      return true;
    }
    if (route === "/api/autonomy/standing-orders/create") {
      if (remoteRequest || managedHost) throw new AutonomyError("Remote clients cannot create standing orders.", 403);
      const body = await readJson(request) as { name?: unknown; scope?: unknown; projectId?: unknown; instruction?: unknown };
      if (typeof body.name !== "string" || typeof body.instruction !== "string" || (body.scope !== "global" && body.scope !== "project")) throw new AutonomyError("Standing order name, scope, and instruction are required.");
      sendJson(response, 200, await autonomy.addStandingOrder({ name: body.name, scope: body.scope, projectId: typeof body.projectId === "string" ? body.projectId : null, instruction: body.instruction }));
      return true;
    }
    if (route === "/api/autonomy/standing-orders/update") {
      if (remoteRequest || managedHost) throw new AutonomyError("Remote clients cannot update standing orders.", 403);
      const body = await readJson(request) as { id?: unknown; name?: unknown; scope?: unknown; projectId?: unknown; instruction?: unknown; enabled?: unknown };
      ...
      const updated = parseStandingOrder({ ...current, ...(typeof body.name === "string" ? { name: body.name } : {}), ...(body.scope === "global" || body.scope === "project" ? { scope: body.scope } : {}), ...(typeof body.projectId === "string" || body.projectId === null ? { projectId: body.projectId } : {}), ...(typeof body.instruction === "string" ? { instruction: body.instruction } : {}), ...(typeof body.enabled === "boolean" ? { enabled: body.enabled } : {}), updatedAt: new Date().toISOString() });
      await state.saveAutonomyRecords({ standingOrders: [updated] });
      sendJson(response, 200, updated);
      return true;
    }
    if (route === "/api/autonomy/standing-orders/delete") {
      if (remoteRequest || managedHost) throw new AutonomyError("Remote clients cannot delete standing orders.", 403);
      const body = await readJson(request) as { id?: unknown };
      if (typeof body.id !== "string") throw new AutonomyError("A standing order is required.");
    ...
    if (route === "/api/autonomy/hooks/create") {
      if (remoteRequest || managedHost) throw new AutonomyError("Remote clients cannot create autonomy hooks.", 403);
      const body = await readJson(request) as { name?: unknown; event?: unknown; flowId?: unknown; projectId?: unknown; cooldownSeconds?: unknown };
      if (typeof body.name !== "string" || typeof body.event !== "string" || typeof body.flowId !== "string") throw new AutonomyError("Hook name, event, and workflow are required.");
      sendJson(response, 200, await autonomy.addHook({ name: body.name, event: body.event as AutonomyHookEvent, flowId: body.flowId, projectId: typeof body.projectId === "string" ? body.projectId : null, cooldownSeconds: typeof body.cooldownSeconds === "number" ? body.cooldownSeconds : undefined }));
      return true;
    }
    if (route === "/api/autonomy/hooks/update") {
      if (remoteRequest || managedHost) throw new AutonomyError("Remote clients cannot update autonomy hooks.", 403);
      const body = await readJson(request) as { id?: unknown; name?: unknown; event?: unknown; flowId?: unknown; projectId?: unknown; enabled?: unknown; cooldownSeconds?: unknown };
      ...
      const updated = parseAutonomyHook({ ...current, ...(typeof body.name === "string" ? { name: body.name } : {}), ...(typeof body.event === "string" ? { event: body.event } : {}), ...(typeof body.flowId === "string" ? { flowId: body.flowId } : {}), ...(typeof body.projectId === "string" || body.projectId === null ? { projectId: body.projectId } : {}), ...(typeof body.enabled === "boolean" ? { enabled: body.enabled } : {}), ...(typeof body.cooldownSeconds === "number" ? { cooldownSeconds: body.cooldownSeconds } : {}), updatedAt: new Date().toISOString() });
      const flow = (await state.load()).autonomyFlows.find((candidate) => candidate.id === updated.flowId);
      if (!flow?.readOnly) throw new AutonomyError("Only built-in read-only workflows can be hooked.", 400);
      await state.saveAutonomyRecords({ hooks: [updated] });
      sendJson(response, 200, updated);
      return true;
    }
    if (route === "/api/autonomy/hooks/delete") {
      if (remoteRequest || managedHost) throw new AutonomyError("Remote clients cannot delete autonomy hooks.", 403);
      const body = await readJson(request) as { id?: unknown };
      if (typeof body.id !== "string") throw new AutonomyError("A hook is required.");
    ...
    if (route === "/api/provider/runs") {
      ...
      void autonomy.dispatch(completed ? "turn_completed" : "turn_failed", persisted.thread.projectId)
        .catch(() => undefined);
      response.end();
      activeAcp.delete(run.id);
      return true;
