...
test("browser MCP advertises bounded tools and forwards bearer authorization", async () => {
  ...
  const broker = createServer((request, response) => {
    ...
    request.on("end", () => {
      ...
      response.end(JSON.stringify(operation.operation?.kind === "snapshot"
        ? {
            ok: true,
            kind: "snapshot",
            snapshot: {
  ...
  const child = spawn(process.execPath, [fileURLToPath(new URL("./browser-mcp.mjs", import.meta.url))], {
    env: {
      ...process.env,
  ...
  try {
    ...
    assert.deepEqual((initialized.result as { capabilities: { tools: unknown } }).capabilities.tools, {});
    ...
    assert.deepEqual(received, { conversationId: "conversation-1", operation: { kind: "snapshot" } });
  } finally {
    child.kill("SIGTERM");
