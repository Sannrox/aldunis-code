...
export function parseProviderAdapterManifest(value: unknown): ProviderAdapterManifest {
  ...
  exact(manifest, [
    "schemaVersion", "id", "publisher", "version", "aldunis", "protocol",
    "executable", "capabilities", "environment", "presentation",
  ], "Adapter manifest");
  ...
  if (
    ...
    throw new ProviderAdapterError(`Adapter is incompatible with Aldunis Code ${ALDUNIS_VERSION}.`, 409);
  }
  ...
  for (const name of names) {
    ...
    if (GENERIC_INTERPRETERS.test(name)) {
      throw new ProviderAdapterError("Executable discovery cannot target a generic interpreter or shell.");
    }
  }
  ...
  const website = presentation.website === undefined
    ? undefined
  ...
  if (website !== undefined) {
    ...
    if (url.protocol !== "https:" || url.username || url.password) {
      throw new ProviderAdapterError("Presentation website must be an HTTPS URL without credentials.");
    }
  }
  return {
    schemaVersion: 1,
...
function safeId(id: string): string {
  if (!ADAPTER_ID.test(id) || id.includes("..")) throw new ProviderAdapterError("Invalid adapter ID.");
  return id;
}
...
export class ProviderAdapterStore {
  ...
  async list(): Promise<InstalledProviderAdapter[]> {
    ...
    for (const entry of entries) {
      ...
      try {
        installed.push((await this.#read(entry.name.slice(0, -5))).current);
      } catch {
        // A corrupt adapter is unavailable rather than silently repaired.
      }
    }
    return installed.sort((left, right) => left.manifest.presentation.name.localeCompare(right.manifest.presentation.name));
  }
  ...
  async version(reference: string): Promise<InstalledProviderAdapter | null> {
    ...
    return [record.current, record.previous].find(
      (candidate) => candidate?.manifest.version === match[2],
    ) ?? null;
  }

  inspect(input: { source: unknown; digest: unknown; manifest: unknown }): InstalledProviderAdapter {
    ...
    if (input.digest !== digest) throw new ProviderAdapterError("Adapter digest does not match the reviewed manifest.");
    return {
      schemaVersion: 1,
      source,
      digest,
  ...
  async install(input: { source: unknown; digest: unknown; manifest: unknown }): Promise<InstalledProviderAdapter> {
    ...
    return this.#serialize(candidate.manifest.id, async () => {
      ...
      if (existing) throw new ProviderAdapterError("An adapter with this ID is already installed.", 409);
      await this.#write(candidate.manifest.id, { schemaVersion: 1, current: candidate, previous: null });
      return candidate;
    });
  }

  async update(input: { source: unknown; digest: unknown; manifest: unknown }): Promise<InstalledProviderAdapter> {
    const candidate = this.inspect(input);
    return this.#serialize(candidate.manifest.id, async () => {
      const existing = await this.get(candidate.manifest.id);
      if (!existing) throw new ProviderAdapterError("Install the adapter before updating it.", 404);
      if (compareVersions(candidate.manifest.version, existing.current.manifest.version) <= 0) {
        throw new ProviderAdapterError("Adapter updates must increase the semantic version.", 409);
      }
  ...
  async setEnabled(id: string, enabled: boolean): Promise<InstalledProviderAdapter> {
    ...
    return this.#serialize(adapterId, async () => {
  ...
  async rollback(id: string): Promise<InstalledProviderAdapter> {
    ...
    return this.#serialize(adapterId, async () => {
      const existing = await this.get(adapterId);
      if (!existing?.previous) throw new ProviderAdapterError("No prior adapter version is available.", 409);
      const current = existing.current;
      const previous = { ...existing.previous, enabled: current.enabled, installedAt: new Date().toISOString() };
      await this.#write(adapterId, { schemaVersion: 1, current: previous, previous: current });
      return previous;
    });
  }
  ...
  async uninstall(id: string): Promise<void> {
    const adapterId = safeId(id);
    await this.#serialize(adapterId, () => rm(this.#path(adapterId)));
  }

  async resolveExecutable(adapter: InstalledProviderAdapter, selectedPath?: string): Promise<string> {
    ...
    if (selectedPath !== undefined) {
      if (!isAbsolute(selectedPath) || !allowedNames.has(parse(selectedPath).base)) {
        throw new ProviderAdapterError("Selected executable does not match the adapter discovery rules.");
      }
  ...
  async #read(id: string): Promise<AdapterRecord> {
    ...
    if (Buffer.byteLength(raw) > MAX_MANIFEST_BYTES * 3) throw new ProviderAdapterError("Stored adapter metadata is oversized.");
    const value = JSON.parse(raw) as AdapterRecord;
    if (value?.schemaVersion !== 1 || !value.current) throw new ProviderAdapterError("Stored adapter metadata is corrupt.", 500);
    const validate = (installed: InstalledProviderAdapter): InstalledProviderAdapter => {
      ...
      if (adapterDigest(manifest) !== installed.digest) {
        throw new ProviderAdapterError("Stored adapter metadata failed integrity verification.", 500);
      }
