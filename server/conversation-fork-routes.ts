...
const FORK_ROUTES = new Set(["/api/forks/preview", "/api/forks/create"]);

/**
 * Dispatches reviewed cross-provider conversation forks behind one interface.
 * The module owns manifest validation, workspace admission, provider readiness,
 * and persistence without changing provider, filesystem, or approval authority.
 */
export async function handleConversationForkRoute(
  ...
  if (!FORK_ROUTES.has(route)) return false;
  const {
    state,
    worktrees,
    profiles,
  ...
  const modelServices: ProviderModelServices = {
    codex,
    shikigami,
